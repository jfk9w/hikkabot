...
// Includes JSON parsing if the schema is not a string.
func getTagValue(s *Schema, t reflect.Type, value string) (interface{}, error) {
	...
	vv := reflect.Indirect(reflect.New(t))
	if err := yaml.NewDecoder(strings.NewReader(value)).Decode(vv.Addr().Interface()); err != nil {
		return nil, err
	}
...
func (s *Schema) ApplyDefaults(v any, post bool) error {
	...
	if s.Default != nil {
		if value.Kind() == reflect.Slice || value.Kind() == reflect.Array || value.Kind() == reflect.Map {
			if post && value.Len() == 0 {
				if err := internal.DecodeAsYAML(s.Default, value.Addr().Interface()); err != nil {
					return errors.Wrapf(err, "decode collection default %#v", s.Default)
				}
			...
			if err := internal.DecodeAsYAML(s.Default, value.Addr().Interface()); err != nil {
				return errors.Wrapf(err, "decode default %#v", s.Default)
			}
	...
	case reflect.Struct:
		for i := 0; i < value.NumField(); i++ {
			...
			if jsonTag, ok := structField.Tag.Lookup("yaml"); ok {
			...
			if IsAnonymous(structField) {
				if err := s.ApplyDefaults(value.Field(i).Addr().Interface(), post); err != nil {
					return errors.Wrapf(err, "in anonymous %s", name)
				}

				continue
			}

			if schema, ok := s.Properties[name]; ok {
				if err := schema.ApplyDefaults(value.Field(i).Addr().Interface(), post); err != nil {
	...
	case reflect.Slice, reflect.Array:
		...
		if s.Items == nil {
			return errors.New("type is slice or array, but no items in schema")
		}

		for i := 0; i < value.Len(); i++ {
			if err := s.Items.ApplyDefaults(value.Index(i).Addr().Interface(), true); err != nil {
				return errors.Wrapf(err, "in index %d", i)
			}
		}
	}
...
func IsAnonymous(field reflect.StructField) bool {
	...
	if strings.HasSuffix(yamlTag, ",inline") {
		return true
	}
...
func GenerateFromField(f reflect.StructField, mode Mode) (string, bool, *Schema, error) {
	jsonTags := strings.Split(f.Tag.Get("yaml"), ",")
	...
	if tag, ok := f.Tag.Lookup("examples"); ok {
		for _, v := range strings.Split(tag, ",") {
			...
			s.Examples = append(s.Examples, parsed)
		}
	}
