...
start() {
    if [[ -f ${RUNFILE} ]]; then
        ...
        CERT=`cat ${CONFIG} | jq -r ".cert"`
        KEY=`cat ${CONFIG} | jq -r ".key"`
        mkdir -p ${LOGDIR}
        env TOKEN=${TOKEN} LOG=${CONFIG} HOST=${HOST} ROOT=${ROOT} HIDDEN_BOARDS=${HIDDEN_BOARDS} DOMAIN=${DOMAIN} CERT=${CERT} KEY=${KEY} hikkabot 2>&1 > ${LOGDIR}/main.log &
        echo -e "PID=$!" > ${RUNFILE}
        notify "RUNNING" 1
    fi
}
